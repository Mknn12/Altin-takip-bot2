...
if not BOT_TOKEN or not API_KEY or not CHAT_ID:
    raise RuntimeError("BOT_TOKEN, API_KEY veya CHAT_ID eksik!")
...
def veri_cek():
    try:
        ...
        altin_data = requests.get(altin_url).json()
        usd_data = requests.get(usd_url).json()

        if not altin_data or 'price' not in altin_data[0]:
            raise ValueError(f"Altın verisi geçersiz: {altin_data}")
        if not usd_data or 'price' not in usd_data[0]:
            raise ValueError(f"USD verisi geçersiz: {usd_data}")

        altin = altin_data[0]['price']
...
# Telegram fırsat bildirimi
async def firsat_analiz_ve_gonder(bot_app):
    altin, usd, puan = veri_cek()
    tahmin = altin_tahmini()
    if altin and tahmin and tahmin > altin * 1.01 and puan > 0.2:
...
async def start_bot():
    ...
    async def periyodik_gorev():
        while True:
            await firsat_analiz_ve_gonder(app)
            await asyncio.sleep(3600)  # 1 saatte bir kontrol

    asyncio.create_task(periyodik_gorev())
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("⛔ Event loop zaten çalışıyor.")
